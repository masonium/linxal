...
impl Eigen for f32 {
    ...
    fn values_mut<D>(mat: &mut ArrayBase<D, (Ix, Ix)>) -> Result<Array<Self::Eigv, Ix>, EigenError> where D:DataMut<Elem=Self> {
        let mut v_empty = [0.0 as f32];
        let mut v_e1 = [0.0 as f32];
        ...
        let info = sgeev(layout, 'N' as u8, 'N' as u8, n, data_slice,
                         n, &mut values_real, &mut values_imag, &mut v_e1, n, &mut v_empty, n);
